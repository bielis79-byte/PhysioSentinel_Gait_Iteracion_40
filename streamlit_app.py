...
def match_biplanar_cycles(front_snapshot, lateral_snapshot, sync_offset_s, tolerance_s=0.22, valid_front=None, valid_lateral=None, physical_window=None):
    """
    ...
    phase=_v39_phase_sync(front_snapshot,lateral_snapshot,master_off)
    phase_q=str(phase.get("phase_quality","No calculable"))
    phase_off=float(phase.get("phase_offset_s",master_off))
    # Sólo usar el refinamiento periódico si está respaldado por >=2 pares y no es "No fiable".
    phase_supported=(int(phase.get("phase_pairs",0))>=2 and phase_q not in ("No fiable","No calculable"))
    match_off=phase_off if phase_supported else master_off
...
# ------------------------- UI -------------------------
st.title("PhysioSentinel Gait")
st.caption(f"Versión {APP_VERSION} · selección multipersona con identidad bloqueada · sincronización biplanar frontal+lateral · Supabase tolerante a fallos")
...
with tabs[0]:
    ...
    if not is_two_cam:
        ...
        st.caption("Las cámaras pueden empezar a grabar en instantes diferentes. Si es posible, deja unos segundos de margen. La Versión 33 estima automáticamente el desfase temporal usando el movimiento del paciente y la ventana de visibilidad compartida; un evento común visible mejora la comprobación, pero no es obligatorio.")
        c1, c2 = st.columns(2)
        with c1:
...
with tabs[3]:
    ...
    if df1 is not None and meta1:
        if current_two and df2 is not None and meta2:
            ...
            if w1 and w2:
                ...
                st.metric("Ventana biplanar detectada", f"{overlap_d:.2f} s", help="Intervalo en el que el mismo paciente puede analizarse simultáneamente desde ambas cámaras tras corregir el desfase.")
            st.caption("FPS (Frames Per Second / Fotogramas por Segundo). La sincronización no exige que ambas cámaras empiecen a grabar a la vez; exige identificar el mismo instante físico en las dos grabaciones.")
            max_sync = 2.0
    ...
    if st.session_state.get("metrics_done"):
        ...
        if st.session_state.get("mode", "").startswith("2 cámaras"):
            ...
            st.markdown("##### Correspondencia temporal frontal-lateral · Versión 40")
            ...
            if ready:
                ...
                for reason in st.session_state.get("ready_3d_reasons", []):
                    st.write(f"• {reason}")
            st.warning("La Versión 33 **no triangula todavía coordenadas 3D (Three-Dimensional / Tridimensional)**. Los resultados siguientes continúan siendo 2D, pero cada plano se interpreta desde la cámara apropiada.")
...
with tabs[6]:
    ...
    st.write("La Versión 33 prepara el flujo para **triangulación 3D (Three-Dimensional / Tridimensional) con Pose2Sim** sin presentar resultados 3D como si estuvieran ya calculados.")
    ...
    if ready:
        st.success("La sesión cumple los criterios de preparación de la Versión 33 para pasar a una futura triangulación 3D (Three-Dimensional / Tridimensional).")
    else:
        st.warning("Preparación 3D incompleta.")
        for reason in reasons:
            st.write(f"• {reason}")
    st.info("Siguiente etapa futura: materializar el perfil de calibración en /tmp, aplicar sincronización, ejecutar triangulación Pose2Sim, filtrar puntos 3D y posteriormente calcular cinemática/OpenSim. La Versión 33 todavía no ejecuta triangulación 3D (Three-Dimensional / Tridimensional) ni cinética estimada.")
...
st.divider()
st.caption("PhysioSentinel Gait Versión 33 · ICLM (Índice de Capacidad Locomotora) 0.1 capacidad locomotora · análisis biomecánico por tracking interpretado según el plano de cámara · valgo absoluto y pie/retropié no penalizadores en frontal · regla de déficit dominante · COM (Centro de Masa estimado 2D) · consistencia paso a paso · anexo completo · exportación")
